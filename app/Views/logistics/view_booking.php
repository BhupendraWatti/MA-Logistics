                        ...
                        <table class="table table-sm mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= number_format((float)$item['actual_weight'], 2) ?>kg</td>
                                    <td><?= $item['length'] ?>x<?= $item['width'] ?>x<?= $item['height'] ?>cm</td>
                                    <td><strong><?= number_format((float)$item['final_chargeable_weight'], 2) ?>kg</strong></td>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    ?>
                        ...
                        <div class="mb-2"><strong>Weight:</strong> <?= number_format((float)($sales['weight'] ?? 0), 2) ?> KG</div>
                        <div class="mb-2"><strong>Rate:</strong> ₹<?= number_format((float)($sales['rate'] ?? 0), 2) ?></div>
                        <hr>
                        <div class="mb-2"><strong>Total Amount:</strong> <span class="text-success fs-5">₹<?= number_format((float)$calculatedTotal, 2) ?></span></div>
                        <hr>
                        <small class="text-muted">
                            <div><strong>Charges Breakdown:</strong></div>
                            DDC: ₹<?= number_format((float)($sales['ddc'] ?? 0), 2) ?> | SSC: ₹<?= number_format((float)($sales['ssc'] ?? 0), 2) ?> | BTC: ₹<?= number_format((float)($sales['btc'] ?? 0), 2) ?><br>
                            FLC: ₹<?= number_format((float)($sales['flc'] ?? 0), 2) ?> | DOC: ₹<?= number_format((float)($sales['doc'] ?? 0), 2) ?> | TCP: ₹<?= number_format((float)($sales['tcp'] ?? 0), 2) ?><br>
                            Inbound TSP: ₹<?= number_format((float)($sales['inbound_tsp'] ?? 0), 2) ?> | Outbound TSP: ₹<?= number_format((float)($sales['outbound_tsp'] ?? 0), 2) ?><br>
                            Utility: ₹<?= number_format((float)($sales['utility_charges'] ?? 0), 2) ?> | X-Ray: ₹<?= number_format((float)($sales['xray_charges'] ?? 0), 2) ?> | ADO: ₹<?= number_format((float)($sales['ado'] ?? 0), 2) ?><br>
                            AWB Agent: ₹<?= number_format((float)($sales['awb_fees_agent'] ?? 0), 2) ?> | AWB Carrier: ₹<?= number_format((float)($sales['awb_fees_carrier'] ?? 0), 2) ?><br>
                            Admin: ₹<?= number_format((float)($sales['admin_charges'] ?? 0), 2) ?> | Delivery Order: ₹<?= number_format((float)($sales['delivery_order_charges'] ?? 0), 2) ?><br>
                            Inbound Handling: ₹<?= number_format((float)($sales['inbound_handling'] ?? 0), 2) ?> | Inbound Storage: ₹<?= number_format((float)($sales['inbound_storage'] ?? 0), 2) ?><br>
                            Outbound Storage: ₹<?= number_format((float)($sales['outbound_storage'] ?? 0), 2) ?> | Misc: ₹<?= number_format((float)($sales['misc_charges'] ?? 0), 2) ?>
                        </small>
                    <?php else: ?>
