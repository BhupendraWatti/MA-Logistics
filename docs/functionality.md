...
* **Consignment click routing**: In All Bookings (`logistics/manage`), clicking the AWB number opens the read-only Booking Details View (`logistics/view/{id}`) matching Dashboard behavior, instead of directly opening the Edit Booking Form. The Actions column retains direct Edit access, and the Details page `< Back` button intelligently preserves All Bookings table filters and pagination.
