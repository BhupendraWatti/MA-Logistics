...
* **WordPress-Style Grid Column Selection**: Added a "Screen Options" collapsable config panel in [manage_bookings.php](file:///D:/Company%20Work/Company%20projects/MAlogistic%20phase%202/app/Views/logistics/manage_bookings.php) to let admins select which columns are displayed in the main bookings list table. Persisted settings in `localStorage` (`malogistics_booking_columns`) to maintain visibility state across pagination and page loads.
* **Customer Form Pincode Auto-Fill**: Implemented dynamic address field auto-filling (City, State, Country) in [_customer_form_fields.php](file:///D:/Company%20Work/Company%20projects/MAlogistic%20phase%202/app/Views/masters/_customer_form_fields.php) based on the entered 6-digit pincode. Configured via the "Google Maps API Settings" section in [settings.php](file:///D:/Company%20Work/Company%20projects/MAlogistic%20phase%202/app/Views/company/settings.php), which reads and writes the `GOOGLE_MAPS_API_KEY` directly to the `.env` file, with an automatic fallback querying the free Indian Postal Pincode Directory API (`api.postalpincode.in`) when no Google Maps key is set.
